...
impl<T: Trait> Module<T> {
    ...
    fn do_transfer(sender: &T::AccountId, to: &T::AccountId, kitty_id: T::KittyIndex) -> DispatchResult {
        ensure!(sender != to, Error::<T>::RequireDifferentAccount);
        Self::owned_kitties((sender, Some(kitty_id))).ok_or(Error::<T>::RequireOwner)?;
