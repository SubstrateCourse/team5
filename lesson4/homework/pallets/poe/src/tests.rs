...
fn create_claim_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(PoeModule::create_claim(Origin::signed(1), claim.clone(), comment.clone(), 10));
...
fn create_claim_failed_when_claim_already_exist() {
    new_test_ext().execute_with(|| {
        let claim = vec![0, 1];
        let comment = vec![0, 1];
        let _ = PoeModule::create_claim(Origin::signed(1), claim.clone(),  comment.clone(),10);

        assert_noop!(
            PoeModule::create_claim(Origin::signed(1), claim.clone(),  comment.clone(), 10),
