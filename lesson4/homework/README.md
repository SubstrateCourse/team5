...
```bash
./target/release/homework purge-chain --dev
```

Start a development chain with:

```bash
./target/release/homework --dev
```
...
This command will firstly compile your code, and then start a local development network. You can also replace the default command (`cargo build --release && ./target/release/homework --dev --ws-external`) by appending your own. A few useful ones are as follow.

```bash
# Run Substrate node without re-compiling
./scripts/docker_run.sh ./target/release/homework --dev --ws-external

# Purge the local dev chain
./scripts/docker_run.sh ./target/release/homework purge-chain --dev
...
You can generate your own Substrate homework based on a particular Substrate
...
# Switch to a particular branch or commit of the Substrate repo your homework based on
...
.maintain/homework-release.sh ../homework.tar.gz
```
