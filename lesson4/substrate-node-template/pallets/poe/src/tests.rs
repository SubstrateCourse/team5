...
fn when_proof_too_long() {
    new_test_ext().execute_with(||{
        ...
        assert_noop!(PoeModule::create_claim(Origin::signed(1), claim.clone()), Error::<Test>::TooLongHash);
...
fn transfer_not_exist_claim() {
    ...
    let dest :<Test as system::Trait>::AccountId = 123456789;
    let claim = vec![0,1];
    assert_noop!(PoeModule::transfer_claim(Origin::signed(1), claim.clone(),dest), Error::<Test>::NoSuchProof);
...
fn not_the_owner_want_to_transfer() {
    new_test_ext().execute_with(||{
        ...
        let claim = vec![0,1];
        assert_ok!(PoeModule::create_claim(Origin::signed(1), claim.clone()));
        assert_noop!(PoeModule::transfer_claim(Origin::signed(2), claim.clone(), dest), Error::<Test>::NotProofOwner);
