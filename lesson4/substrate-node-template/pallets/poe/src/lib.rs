...
decl_storage! {
	...
	trait Store for Module<T: Trait> as poeMoudle {
		...
		UserInfo get(fn user_info) : map hasher(blake2_128_concat) T::AccountId => Vec<Vec<u8>>;
	}
}
...
decl_event!(
	...
	where AccountId = <T as system::Trait>::AccountId, 
		...
		ClaimCreated(AccountId, Vec<u8>,BlockNumber,Vec<u8>),
...
decl_module! {
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		#[weight = 10_000]
		pub fn create_claim(origin, proof: Vec<u8>, comment:Vec<u8>) -> dispatch::DispatchResult {
			...
			ensure!(T::MaxCommentLength::get() >= comment.len() as u32, Error::<T>::TooLongComment);//limit size for comment
			let current_block = <system::Module<T>>::block_number();
			Proofs::<T>::insert(&proof, (&sender,current_block.clone(),comment.clone()));
			let mut cur_userinfo = Self::user_info(&sender);
			cur_userinfo.push(proof.clone());
			UserInfo::<T>::insert(&sender,cur_userinfo);
			Self::deposit_event(RawEvent::ClaimCreated(sender, proof,current_block, comment));
		...
		pub fn revoke_claim(origin, proof:Vec<u8>) -> dispatch::DispatchResult {
			...
			let mut cur_userinfo = Self::user_info(&sender);
			if let Ok(index) =cur_userinfo.binary_search(&proof){
				
					cur_userinfo.remove(index);
					UserInfo::<T>::insert(owner,cur_userinfo);
				}
		...
		#[weight = 10_000]
		pub fn transfer_claim(origin, proof:Vec<u8>, new_owner:T::AccountId, comment:Vec<u8>) -> dispatch::DispatchResult{
			...
			Proofs::<T>::insert(&proof, (&new_owner,<system::Module<T>>::block_number(),comment.clone()));//cover the old proof cliam
			let mut cur_userinfo = Self::user_info(&new_owner);
			cur_userinfo.push(proof.clone());
			UserInfo::<T>::insert(&new_owner,cur_userinfo);
			let mut old_cur_userinfo =Self::user_info(&sender);
			if let Ok(index) =old_cur_userinfo.binary_search(&proof){
				
					old_cur_userinfo.remove(index);
					UserInfo::<T>::insert(owner,old_cur_userinfo);
				}
		...
		#[weight = 0]
		pub fn buy_claim(origin, claim: Vec<u8>, amount: BalanceOf<T>, comment:Vec<u8>) -> dispatch::DispatchResult {
			let sender = ensure_signed(origin)?;
			ensure!(Proofs::<T>::contains_key(&claim), Error::<T>::NoSuchProof);
			...
			Proofs::<T>::insert(&claim, (sender.clone(), system::Module::<T>::block_number(),comment.clone()));
			let mut cur_userinfo = Self::user_info(&sender);
			cur_userinfo.push(claim.clone());
			UserInfo::<T>::insert(&sender,cur_userinfo);
			let mut old_cur_userinfo =Self::user_info(&owner);
			if let Ok(index) =old_cur_userinfo.binary_search(&claim){
