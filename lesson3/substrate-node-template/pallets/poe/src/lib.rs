...
decl_event!(
	pub enum Event<T> where AccountId = <T as system::Trait>::AccountId, Price = BalanceOf<T> {
		...
		ClaimSetPrice(AccountId, Vec<u8>, Price),
		ClaimSold(AccountId, AccountId, Vec<u8>, Price),
	}
);
...
decl_module! {
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		pub fn set_claim_price(origin, claim: Vec<u8>, price: BalanceOf<T>) -> dispatch::DispatchResult {
			...
			Self::deposit_event(RawEvent::ClaimSetPrice(sender, claim, price));
		...
		pub fn buy_claim(origin, claim: Vec<u8>, amount: BalanceOf<T>) -> dispatch::DispatchResult {
			...
			ensure!(Prices::<T>::contains_key(&claim), Error::<T>::PriceNotExist);

			let price = Prices::<T>::get(&claim);

            ensure!(amount >= price, Error::<T>::InvalidPrice);

            let (owner, _block_number) = Proofs::<T>::get(&claim);

            T::Currency::transfer(&sender, &owner, price, AllowDeath)?;

			Proofs::<T>::insert(&claim, (&sender, system::Module::<T>::block_number()));
            Prices::<T>::remove(&claim);

			Self::deposit_event(RawEvent::ClaimSold(owner, sender, claim, price));
