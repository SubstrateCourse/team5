...
impl<T: Trait> Module<T> {
	...
	fn fetch_price_cryptocompare() -> Result<u32, Error::<T>> {
		debug::info!("fetch cryptocompare");
		let remote_url_bytes = CRYPTOCOMPARE_REMOTE_URL.to_vec();
		...
		let price_info: Cryptocompare = serde_json::from_str(&body_str).unwrap();

		debug::warn!("Submit Signed: Got price: {} ", price_info.usd);

		Ok(price_info.usd)
	}

	fn fetch_price_coincap() -> Result<u32, Error::<T>> {
		debug::info!("fetch coincap");
		let remote_url_bytes = COINCAP_REMOTE_URL.to_vec();
